...
export function TodoProvider({ children }) {
  ...
  const isLoading = useMemo(() => loadTodoStatus?.stattus === 'LOADING', [loadTodoStatus]);
  const hasError = useMemo(
    () => loadTodoStatus?.stattus === 'ERROR' && loadTodoStatus,
    [loadTodoStatus],
  );

  const AddTodoStatus = errorList.find(x => x.task === 'ADD_TODO');

  const isAdding = useMemo(() => AddTodoStatus?.stattus === 'LOADING', [AddTodoStatus]);
